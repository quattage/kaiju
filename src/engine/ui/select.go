...
func (s *Select) Init(text string, options []SelectOption) {
	...
	{
		// Up/down triangle
		triTex, _ := host.TextureCache().Texture(inputAtlas, textures.TextureSamplerModeRepeat, textures.TextureFilterLinear)
		tri := man.Add()
		img := tri.ToImage()
...
func (s *Select) AddOption(name, value string) {
	...
	panel := man.Add()
	p := panel.ToPanel()
	bg, _ := s.Base().Host().TextureCache().Texture(assets.TextureSquare, textures.TextureSamplerModeRepeat, textures.TextureFilterLinear)
	...
	optionIndex := len(data.options)
	{
		...
		arrowTexture, _ := s.Base().Host().TextureCache().Texture(
			inputAtlas, textures.TextureSamplerModeRepeat, textures.TextureFilterLinear)
